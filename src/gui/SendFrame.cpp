...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("catalyst://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "catalyst:");
    }
    if(!_request.startsWith("catalyst:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with catalyst:"), QtCriticalMsg));
      return;
    }
